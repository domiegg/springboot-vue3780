<template>
	<div class="main-content" :style='{"minHeight":"calc(100vh - 200px)","padding":"20px","margin":"0 auto","color":"#666","background":"#f5f5f5","width":"calc(100% - 60px)","fontSize":"14px","height":"100%"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							...
							<el-button class="btn8" v-if="isAuth('shiyanshi','预约')" @click="yuyueshenqingCrossAddOrUpdateHandler(scope.row,'cross','','','','')" type="success">
		...
		<yuyueshenqing-cross-add-or-update v-if="yuyueshenqingCrossAddOrUpdateFlag" :parent="this" ref="yuyueshenqingCrossaddOrUpdate"></yuyueshenqing-cross-add-or-update>
...
import yuyueshenqingCrossAddOrUpdate from "../yuyueshenqing/add-or-update";
	export default {
		data() {
			return {
				searchForm: {
				...
				yuyueshenqingCrossAddOrUpdateFlag: false,
		...
		methods: {
			...
			yuyueshenqingCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				this.showFlag = false;
				this.addOrUpdateFlag = false;
				this.yuyueshenqingCrossAddOrUpdateFlag = true;
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.yuyueshenqingCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.yuyueshenqingCrossaddOrUpdate.init(row.id,type);
				});
			},
