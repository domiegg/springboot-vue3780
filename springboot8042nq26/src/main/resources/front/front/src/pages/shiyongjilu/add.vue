    ...
    >
          ...
          <el-form-item :style='{"width":"45%","padding":"10px","margin":"0 2% 10px","background":"#fff","display":"inline-block"}' label="实验室类型" prop="shiyanshileixing">
            <el-input v-model="ruleForm.shiyanshileixing" 
                placeholder="实验室类型" clearable :disabled=" false  ||ro.shiyanshileixing"></el-input>
          </el-form-item>
          ...
          <el-form-item :style='{"width":"45%","padding":"10px","margin":"0 2% 10px","background":"#fff","display":"inline-block"}' label="使用人数" prop="shiyongrenshu">
            <el-input v-model.number="ruleForm.shiyongrenshu" 
                placeholder="使用人数" clearable :disabled=" false  ||ro.shiyongrenshu"></el-input>
          </el-form-item>
          <el-form-item :style='{"width":"45%","padding":"10px","margin":"0 2% 10px","background":"#fff","display":"inline-block"}' label="使用时间" prop="shiyongshijian">
              <el-date-picker
				  :disabled="true  ||ro.shiyongshijian"
          ...
          <el-form-item :style='{"width":"45%","padding":"10px","margin":"0 2% 10px","background":"#fff","display":"inline-block"}' label="使用记录" prop="shiyongjilu">
            <editor 
...
<script>
  export default {
    ...
    // 多级联动参数
      // 多级联动参数
      info() {
        this.$http.get(`shiyongjilu/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
			...
			this.$refs["ruleForm"].validate(valid => {
				if(valid) {
					...
					if(crossrefid && crossuserid) {
						...
						this.$http.get('shiyongjilu/list', {
							params: params
						}).then(res => {
							if(res.data.data.total>=crossoptnum) {
								...
								this.$http.post(`shiyongjilu/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(res => {
									if (res.data.code == 0) {
										this.$message({
											message: '操作成功',
											type: 'success',
