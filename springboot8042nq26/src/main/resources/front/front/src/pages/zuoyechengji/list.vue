...
<div>
    ...
    <el-form :inline="true" :model="formSearch" class="list-form-pv" :style='{"padding":"10px","margin":"20px 0 0","alignItems":"center","flexWrap":"wrap","background":"#fff","display":"flex","width":"calc(100% - 0px)","height":"auto","order":"2"}'>
      ...
      <el-form-item :style='{"margin":"5px 10px"}'>
	    <div class="lable" v-if="true" :style='{"width":"auto","padding":"0 10px","lineHeight":"42px","display":"inline-block"}'>作业类型：</div>
        <el-input v-model="formSearch.zuoyeleixing" placeholder="作业类型" @keydown.enter.native="getList(1, curFenlei)" clearable></el-input>
      </el-form-item>
	  ...
	  <el-button v-if="btnAuth('zuoyechengji','新增')" :style='{"cursor":"pointer","border":"1px solid #170000","padding":"0px 15px","margin":"0 10px 0 0","outline":"none","color":"#000","borderRadius":"4px","background":"#fff","width":"auto","fontSize":"14px","lineHeight":"42px","height":"42px"}' type="primary" @click="add('/index/zuoyechengjiAdd')"><i v-if="true" :style='{"color":"#000","margin":"0 10px 0 0","fontSize":"14px"}' class="el-icon-circle-plus-outline"></i>添加</el-button>
    </el-form>
		  ...
		  <div v-for="(item, index) in dataList" :key="index" @click.stop="toDetail(item)" class="list-item animation-box">
		    ...
		    <div :style='{"padding":"0 10px","overflow":"hidden","alignItems":"center","flex":"1","display":"flex","height":"auto"}' class="item-info">
		      <div :style='{"width":"auto","padding":"0 10px 0 0","flexDirection":"column","flex":"1","display":"flex"}'>
				...
				<div class="title">{{item.zuoyeleixing}}</div>
				<div class="title">分数:{{item.fenshu}}</div>
		      ...
		      <div class="desc" v-html="item.pingyu"></div>
		    </div>
		  </div>
		</div>
	</div>
...
<script>
  export default {
    ...
    created() {
		...
		this.selectOptionsList.push({name:'成绩',list:this.chengjiOptions,tableName: 'chengji',check: -1})
      this.getFenlei();
      this.getList(1, '全部');
    },
    //方法集合
    methods: {
      ...
      getList(page, fenlei, ref = '') {
        ...
        if (this.formSearch.zuoyeleixing != '') searchWhere.zuoyeleixing = '%' + this.formSearch.zuoyeleixing + '%';
        if (this.formSearch.chengji != '') searchWhere.chengji = this.formSearch.chengji;
			let user = JSON.parse(localStorage.getItem('sessionForm'))
		if (this.sortType) searchWhere.sort = this.sortType
		if (this.sortOrder) searchWhere.order = this.sortOrder
        this.$http.get(`zuoyechengji/${this.centerType?'page':'list'}`, {params: Object.assign(params, searchWhere)}).then(res => {
          if (res.data.code == 0) {
      ...
      toDetail(item) {
        ...
        this.$router.push({path: '/index/zuoyechengjiDetail', query: params});
      },
